...
class TestDataCleaning(unittest.TestCase):
    ...
    def test_clean_scraper_data(self):
        ...
        data = [
            Row(product_id=1.0, marketplace_name="Amazon", product_name="Laptop_Pro", brand=None, category="Electronics", condition="New", resale_price=999.9, seller_name="SellerA", seller_rating=4.5, location="US", listing_date="2024-01-01", product_url="http://test", availability_status="In Stock", scraped_date="2024-01-02"),
        ...
        # Verify null brand is replaced with "unknown" (lowercase) and underscores replaced with spaces
        result = df_clean.collect()[0]
        self.assertEqual(result["brand"], "unknown")
        self.assertEqual(result["product_name"], "laptop pro")
    ...
    def test_clean_warranty_details(self):
        ...
        # Verify null optional field is filled with "unknown" and IDs are uppercased
        rows = df_clean.filter(col("warranty_id") == "W3").collect()
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0]["warranty_type"], "unknown")
        self.assertEqual(rows[0]["sku_id"], "SKU3")
    ...
    def test_clean_bom_details(self):
        ...
        self.assertEqual(row_b3["sku_id"], "SKU3")

    def test_clean_sku_master(self):
        schema = get_sku_master_schema()
        data = [
            Row(sku_id="sku1", product_id=1.0, product_name="Laptop_Pro", brand="BrandA", category="CategoryA", model_number="M1", original_price=999.0, manufacturing_date="2024-01-01", launch_year=2024, product_type="TypeA", material_type="Metal", weight=2.5, dimensions="10x5", country_of_origin="US", expected_life_span=5, repairability_score=8.5),
            # Null sku_id (dropped)
        ...
        self.assertEqual(row_sku3["product_name"], "laptop")
